...
## 数据交换、系统回调、异步任务和写操作边界

触发：

- 路径、参数、JS、文档或错误信息出现 `sync`、`callback`、`webhook`、`notify`、`task`、`job`、`queue`、`event`、`openapi`、`appkey`、`secret`、`signature`、`nonce`、`timestamp`。
- 接口语义涉及导入、导出、对账、批处理、报表、任务调度、消息发布/消费、第三方通知、机器到机器调用或跨系统状态回写。
- 发现核心对象的写操作、审批、删除、状态流转、任务触发、设备/媒体控制、配置修改或管理动作。

最小验证：

1. 建立认证态、未认证态、错误签名、空签名、错误 `Content-Type` 和错误 method 对照。
2. 优先用只读请求、空 body、空 JSON、空数组、fake-object、测试前缀对象判断是否进入业务逻辑。
3. 记录状态码、错误栈、处理器名称、业务字段、任务 ID、签名校验差异和权限错误差异。
4. 不提交真实订单、财务、合同、工单、用户、业务状态、设备控制或其它生产业务数据。
5. 需要真实对象、测试账号、测试 AppKey/Secret、测试设备或日志侧确认时，标记 `blocked_need_material`。

写操作边界验证层级：

```text
L1：OPTIONS / GET / 空 POST / 错误 Content-Type，判断接口是否需要认证和签名。
L2：空 JSON、空数组、fake-object、测试前缀对象，证明是否进入业务处理分支。
L3：仅在用户提供测试对象、默认授权写入和可回滚条件时，执行一次最小真实写入。
L4：成功证明一次即停止，不批量、不触碰真实业务对象、不扩大到生产全量同步。
```

停止点：证明认证缺失、签名未校验、进入业务处理器、可触发任务分支、可访问内部配置或可对测试对象完成一次可回滚动作后停止。

需要材料时输出：低权限/高权限账号对、测试对象 ID、测试租户/组织、测试 AppKey/Secret、测试文件、测试设备、可回滚对象、甲方日志确认方式。
