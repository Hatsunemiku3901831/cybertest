...
```bash
git clone https://github.com/<your-org>/cybertest.git
cd cybertest
```
...
2. 使用codex或者claude code，直接使用自然语言提出需求
...
- 报告、日志和归档应保留可复现证据，但公开发布前必须移除真实客户数据、凭据、token、cookie、个人路径和其它敏感信息。
- 面向 GitHub 的 README 不应引用本地任务归档、临时文件、缓存文件或未跟踪资产。
